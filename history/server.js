...
const { cookie, check } = require('express-validator');
...
app.get('/readhistoryDBuser', async (req, res) => {
    console.log("444444");
        ...
        INNER  JOIN hotel_profile ON booking_info.hotel_id=hotel_profile.hotel_id WHERE booking_info.user_id = 2`;//join 2รอบ
        let result = await queryDB(sql);
        result = Object.assign({}, result);
        console.log(result);
        res.json(result);
...
app.get('/readhistoryDBhost', async (req, res) => {
    ...
    INNER JOIN user_profile ON booking_info.user_id=user_profile.user_id
    INNER  JOIN hotel_profile ON booking_info.hotel_id=hotel_profile.hotel_id WHERE booking_info.hotel_id = 1`;//join 2รอบ
    let result = await queryDB(sql); 
    result = Object.assign({}, result);
    console.log(result);
    res.json(result);
