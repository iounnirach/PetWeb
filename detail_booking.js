...
function pageLoad() {
    ...
    getbooking_info();
    gethotel_info();
    document.getElementById("cancel_booking").onclick = delete_dataBooking;
    document.getElementById("close").onclick = close_dataBooking;
}
...
async function gethotel_info() {
    ...
    const response2 = await fetch("\chackdata_hotelInfo_tell");
    const content = await response.json()
    const content2 = await response2.json()
    createdata_hotelInfo(content, content2);
}

function createdata_hotelInfo(data, data2) {
...
function close_dataBooking() {
    window.location.assign("Booking_guest_page.html");
    // res.end();
}